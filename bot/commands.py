...
async def cmd_categories(message: Message):
    ...
    try:
        ...
        # Формируем сообщение
        response = ["📋 <b>Доступные категории:</b>\n"]
        
        if expense_categories:
            response.append("<b>Категории расходов:</b>")
            for cat in expense_categories:
                response.append(f"{cat.emoji} {cat.name.capitalize()}")
        else:
            response.append("Категории расходов пока не созданы")
        ...
        response.append("\nДля добавления новой категории просто используйте её в транзакции")
        
        await message.answer("\n".join(response), parse_mode=ParseMode.HTML)
    except Exception as e:
        logging.error(f"Ошибка при обработке команды /categories: {e}")
        await message.answer("Произошла ошибка при получении списка категорий. Попробуйте позже.")
    finally:
        db.close()
