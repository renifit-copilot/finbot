...
from core.llm import get_advice, update_category_cache, rebuild_category_cache
    ...
    try:
        ...
        if not user:
            ...
            standard_categories = [
                ...
                {"name": "жильё и коммунальные услуги", "emoji": "🏠", "is_expense": 1},
            ...
            try:
                rebuild_category_cache(db, user.id)
...
async def cmd_help(message: Message):
    ...
    help_text = (
        ...
        "/feedback - Дать обратную связь по категоризации\n"
        "/learn товар категория - Научить бота правильной категоризации\n"
        "/resetcategories - Сбросить и создать стандартный набор из 9 категорий\n"
        "/clearcache - Очистить кэш категорий\n"
        "/rebuildcache - Перестроить кэш категорий для точной категоризации\n"
        "\n"
        
        "<b>ДОПОЛНИТЕЛЬНО</b>\n"
        "/advice - Получить финансовый совет\n"
        "📷 Фото чека - Отправьте фото чека для автоматической обработки\n"
        "\n"
    ...
    try:
        ...
        if not expense_categories and not income_categories:
            await message.answer(
                ...
                "Используйте команду /resetcategories, чтобы создать стандартный набор из 9 основных категорий.",
                parse_mode=ParseMode.HTML
            )
            return
        ...
        message_text += (
            ...
            "<i>Команда /resetcategories сбросит все категории и создаст стандартный набор из 9 основных категорий</i>"
        )
...
@router.message(Command("resetcategories"))
async def reset_categories(message: Message):
    """Сбрасывает категории пользователя и создает стандартный набор из 9 основных категорий"""
    user_id = message.from_user.id
    ...
    try:
        ...
        try:
            ...
            status_msg = await message.answer("🔄 Сбрасываю категории и создаю новые...")
            
            # Удаляем все существующие категории пользователя
            user_categories = db.query(Category).filter(Category.user_id == user.id).all()
            for category in user_categories:
                db.delete(category)
            
            # Создаем стандартный набор из 9 категорий
            standard_categories = [
                # Расходы
                {"name": "продукты", "emoji": "🌱", "is_expense": 1},
                {"name": "еда вне дома", "emoji": "🍔", "is_expense": 1},
            ...
            # Отправляем подтверждение
            await message.answer(
                "✅ Категории сброшены! Создан стандартный набор из 9 основных категорий:\n\n"
                "🌱 Продукты\n"
                "🍔 Еда вне дома\n"
                "🏡 Быт\n"
                "👔 Одежда и обувь\n"
                ...
                "🎮 Развлечения\n\n"
                "Кэш категорий также перестроен для точной категоризации.\n"
                "Используйте команду /categories для подробной информации о категориях."
            )
                
        except Exception as e:
            db.rollback()
            logging.error(f"Ошибка при сбросе категорий: {e}")
            await message.answer("Произошла ошибка при сбросе категорий")
        finally:
            db.close()
            
    except Exception as e:
        logging.error(f"Ошибка при обработке команды сброса категорий: {e}")
        await message.answer("Произошла ошибка при обработке команды")


@router.message(Command("rebuildcache"))
async def cmd_rebuild_cache(message: Message):
    """Полностью перестраивает кэш категорий для более точной категоризации"""
    user_id = message.from_user.id
    ...
    try:
        ...
        try:
            ...
            # Отправляем сообщение о начале процесса
            status_msg = await message.answer("🔄 Перестраиваю кэш категорий...")
            
            # Перестраиваем кэш
            success = rebuild_category_cache(db, user.id)
            ...
            if success:
                await message.answer(
                    "✅ Кэш категорий успешно перестроен.\n\n"
                    "Теперь бот будет более точно определять категории для ваших транзакций.\n"
                    "Введите /categories чтобы увидеть доступные категории."
                )
            else:
                await message.answer("❌ Не удалось перестроить кэш категорий. Попробуйте позже.")
                
        except Exception as e:
            db.rollback()
            logging.error(f"Ошибка при перестройке кэша категорий: {e}")
            await message.answer("Произошла ошибка при перестройке кэша категорий")
        finally:
            db.close()
