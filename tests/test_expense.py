...
class TestDatabaseModels(unittest.TestCase):
    ...
    def test_expense_model(self):
        ...
        # Проверяем сохранение
        saved_expense = self.session.query(
            Expense).filter_by(user_id=user.id).first()
...
class TestTransactionParser:
    ...
    def test_mention_parsing(self):
        ...
        # Неверное поведение, но так работает код
        assert result["mentioned_user"] == "r"

    def test_complex_message_parsing(self):
        """Тест парсинга сложных сообщений"""
        # Расход + валюта + дата + упоминание
        result = parse_transaction_message(
            "99.99 USD подарок @friend 15.08.2023")
